...
class BiLSTM_CRF:
    ...
    def __init__(self, tagset_size, num_lstm_layers, hidden_dim, word_embeddings, morpheme_embeddings, morpheme_projection, morpheme_decomps, train_vocab_ctr, vocab_size=None, word_embedding_dim=None):
    ...
    def forward(self, observations):
        ...
        def log_sum_exp(scores):
            ...
            #ix = scores - max_score_expr_broadcast
...
i2t = { i: t for t, i in list(t2i.items()) }
i2c = { i: c for c, i in list(c2i.items()) }
i2mt = { i: mt for mt, i in list(mt2i.items()) }

tag_list = [ i2t[idx] for idx in range(len(i2t)) ] # To use in the confusion matrix
mtag_list = [ i2mt[idx] for idx in range(len(i2mt)) ] # because why not
...
if options.no_sequence_model:
    ...
    model = BiLSTM_CRF(len(t2i), options.lstm_layers, options.hidden_dim,
                       word_embeddings, morpheme_embeddings, morpheme_projection, morpheme_decomps, training_vocab,
                       vocab_size = len(w2i), word_embedding_dim = 128)
